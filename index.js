...
const cep = JSON.parse(fs.readFileSync('./src/cep.json'))
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const premium = ["5511911219615@s.whatsapp.net","17198903218@s.whatsapp.net","12603386264@s.whatsapp.net","555196953702@s.whatsapp.net"]
...
`
          ...
          reply('Espere..')
				...
				case 's':
				case 'ovo':
					if (!isNsfw) return reply('❗𝗖𝗼𝗺𝗮𝗻𝗱𝗼 𝗱𝗶𝗽𝗼𝗻𝗶𝘃𝗲𝗹 𝗮𝗽𝗲𝗻𝗮𝘀 𝗻𝗼 𝗽𝘃❗')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
